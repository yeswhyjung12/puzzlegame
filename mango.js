...
function setupBoard() {
    ...
    let board = document.getElementById("mangoBoard");
    if (board) {
        ...
        for (let r = 0; r < rows; r++) {
            for (let c = 0; c < columns; c++) {
                ...
                tile.src = "mangoImg/" + imgOrder.shift() + ".jpg";
                ...
                turns = 0;
                document.getElementById("mangoTurns").innerText = turns;
                document.getElementById("overlay5-turns").innerText = turns;
                document.getElementById("overlay5").style.display = "none"; // 오버레이를 숨김

                console.log('Image added:', tile.src); // 디버그 메시지 추가
            }
        }
    } else {
        console.error('Element with id "mangoBoard" not found');
    }
}

console.log('mango.js script ended');
...
function swapTiles() {
    ...
    if (isAdjacent) {
        ...
        if (checkCompletion()) {
            ...
            setTimeout(() => {
                document.getElementById("overlay5").style.display = "flex"; // 오버레이를 표시
            }, 700); // 0.7초 지연
        }
    }
}
...
function disableTiles() {
    let tiles = document.querySelectorAll("#mangoBoard img");
    tiles.forEach(tile => {
